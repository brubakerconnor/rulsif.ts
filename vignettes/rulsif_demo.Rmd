...
Nothing too different still except just at the start of the series.

```{r}
detect_alpha_0.75 <- ts_detect(series, alpha = 0.75, make_plot = TRUE)
```

This time it looks like we've picked up that small mean change towards the start.
Now for the last alpha setting.

```{r}
detect_alpha_1 <- ts_detect(series, alpha = 0.99, make_plot = TRUE)
```

Alright, so nothing truly exciting happened by varying the alpha parameter. But this
is probably because we may not have the issues of an unbounded ratio with this
particular series.
